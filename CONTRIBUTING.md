...
- **Épave échouée** (Jour 7, **toujours déclenché**) : offre un don de **Bois brut** (`bois_base`/`bois_min`, `computeEpaveBoisBonus()`), `max(bois_min, bois_base − V)` — soit 19−V plafonné à 5 minimum, même forme dégressive que les autres bonus V-dépendants (petit groupe = bonus plus généreux, pour compenser le fait qu'un petit groupe a structurellement moins de votes/jour à consacrer au Radeau).
  ⚠️ **Inversion du 28/08** : à l'origine, Épave donnait des points de Radeau **directs** (`max(points_min, points_base − V)`, soit 26−V plafonné à 10), en s'appuyant sur le raisonnement que "convertir du bois en points coûte encore un vote, donc un don de bois seul n'aide pas le goulot d'étranglement réel (le nombre de votes disponibles, pas la ressource)". Ce raisonnement s'est avéré faux en pratique : avec un groupe qui avait déjà 9 points en banque au Jour 5, le don direct rendait la victoire par Radeau **quasi certaine dès le Jour 7** (jusqu'à +22 points d'un coup, largement plus que les 11 restants) — le Bois n'était PAS le facteur limitant dans une partie qui a une économie plutôt généreuse (stocks_initiaux confortables + bonus Jour 5/Colis Royal), donc le don direct contournait entièrement le vrai frein du jeu. Le don de Bois réintroduit ce frein : il faut encore de vrais votes Radeau (1 vote = 1 point, `bois_par_point_radeau: 1`) pour le convertir, en concurrence avec les besoins de survie du jour.
