...
2. **Jour de jeu** (jours 1 à 10) : chaque joueur vivant choisit un lieu (bouton éphémère), qui détermine son action — vote au Château, combat au Camp d'Entraînement, enquête à la Tour de Guet, protection à la Taverne, révélation de position à la Clairière mystique. **Définitif dès validation** (`isActionLocked()`, aucun changement d'avis possible sur aucun lieu — voir plus bas), enregistré via `recordAction()` en `HSET` (écrasable seulement tant qu'aucune valeur n'a encore été écrite pour ce slot). Rien n'est publié publiquement en cours de journée.
