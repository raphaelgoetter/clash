...
⚠️ **Princesse — exception assumée du Jour 3 (08/09)** : demande explicite de faire de la Princesse le cœur d'un événement, en évitant que le texte narratif ne le rende trop lisible. Ajout du champ `princesse_multiplier` (défaut 1, comme `sorcierMultiplier`/`archeresMultiplier`), lu par `resolveDayParams()` et appliqué UNIQUEMENT dans `computeComboDamage()` (`princesseTotal = princesseCount * degats * princesseMultiplier`) — le reste du moteur reste inchangé. Réintroduit délibérément, pour ce seul jour, le comportement « strictement dominante » identifié et évité ci-dessus : à `princesse_multiplier: 3` (75 dégâts effectifs) combiné à un Sorcier neutralisé (`sorcier_multiplier: 0`) et une Archères affaiblie (`archeres_multiplier: 0.8`), la Princesse se prend systématiquement au plafond (10 votes) quel que soit N — vérifié stable à N=15 et N=18 (`1🛡️ 2-5🗡️ 2🏹 10👑`). Le texte de l'événement (« Brouillard Occultant ») explique en clair l'effet Sorcier/Archères mais ne mentionne jamais la Princesse, pour laisser sa force le mystère qu'elle doit rester — cohérent avec la règle générale « aucun événement listé à l'avance ».
...
- **🌫️ Brouillard Occultant** (Jour 3, remplace Frappe Léthale le 08/09) : Sorcier neutralisé (`sorcier_multiplier: 0`), Archères légèrement affaiblies (`archeres_multiplier: 0.8`), et Princesse boostée x3 (`princesse_multiplier: 3`, 75 dégâts effectifs — **jamais évoqué dans la description**, voir "Princesse — exception assumée du Jour 3" ci-dessus) — combo optimale stable à `1🛡️ 2-5🗡️ 2🏹 10👑`, Princesse toujours au plafond.
