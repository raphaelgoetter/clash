...
Chaque Chevalier protège jusqu'à `protectionSlots` unités à distance (Sorcier/Archères), potentiellement réduit par l'événement du jour (`protection_slots_override`). Si le nombre de distants dépasse la capacité, les slots vont aux votants dont le vote a été fixé/mis à jour le **plus tôt** ce jour-là (`computeProtection()`, tri par `bossraid:vote_at:<jour>` croissant) — approximation la plus fidèle d'un « ordre d'arrivée » alors que les votes sont modifiables jusqu'au cron. Non protégé : malus du jour (`malusMultiplier`, 0.5 par défaut, jusqu'à 0 lors de Frappe Léthale) ; protégé : jamais de malus, quel que soit l'événement.
