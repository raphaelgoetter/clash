    ...
    await Promise.race([
      Promise.allSettled(
        rivalClans.map(async (c) => {
          ...
          const currProm = fetchCurrentRace(c.tag)
            .then((res) => {
              rivalCurrentRaceByTag[tagNorm] = res;
            })
            .catch((e) =>
              console.warn(
                `[clan]     ! fetchCurrentRace ${tagNorm} failed:`,
                e.message,
              ),
            );
      ...
      ? (() => {
          ...
          const groupWithProjections = currentRace.clans.map((c) => {
            ...
            if (isWarPeriod && raceData?.clan?.participants) {
              ...
              // Fame officielle du clan pour la période en cours (source: clans[i].fame)
              // Cohérent avec cwstats : fame_actuelle / decks_aujourd'hui
              const clanFame = c.fame ?? 0;
              ...
              if (decksToday > 0) {
                // Efficacité (E) : fame officielle ÷ decks joués aujourd'hui (formule cwstats)
                ptsPerDeck = clanFame / decksToday;
