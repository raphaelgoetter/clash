...
export async function analyzePlayer(
  ...
  const reliability = computeWarReliabilityFallback(
    player,
    warLog,
    battleLogBreakdown,
    lastSeen,
  ...
  if (nextTourReq && tourLevel < nextTourLevel) {
    ...
    tourNextInfo = {
      missing: Math.max(0, nextTourReq.cards - have),
      level: nextTourReq.level,
    };
  }
...
export async function getPlayerAnalysis(tag, discordLinked = false) {
  ...
  if (currentClanTag || hasBattleLogClanTag) {
    try {
      ...
      const warWinRate =
        rawWarLog.length >= 10 ? gdcWins / rawWarLog.length : null;
      ...
      const effectiveWinRate =
        analysis.warHistory.historicalWinRate ?? warWinRate;

      if (hasEnoughHistory) {
        analysis.warScore = computeWarScore(
          player,
          analysis.warHistory,
          effectiveWinRate,
          lastSeen,
        ...
        analysis.warScore = computeWarReliabilityFallback(
          player,
          warLogFb,
          bdFb,
          lastSeen,
  ...
  const currentWeek =
    analysis.warHistory?.weeks?.find((w) => w.isCurrent) ?? null;
  const raceTotalDecks = currentWeek?.decksUsed ?? null;
  const warSummary = buildCurrentWarDays(
    battleLog,
    raceTotalDecks,
    currentRaceMeta,
    player.clan?.tag ?? null,
  );
 ...
 */
export function buildCurrentWarDays(
  battleLog,
  raceTotalDecks = null,
  raceMeta = null,
  clanTag = null,
  ...
  const fallbackDaysFromThu = isWarPeriod
    ? nowDow === 4
      ? 0
      : nowDow === 5
        ? 1
  ...
  const totalDecksUsed = isReliableTotal
    ? raceTotalDecks
