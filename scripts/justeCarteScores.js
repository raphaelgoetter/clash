#!/usr/bin/env node
// justeCarteScores.js
// Affiche le classement de la partie La Juste Carte en cours : joueur,
// nombre de tentatives, score de cette partie et score total de la saison.
//
// Usage : node scripts/justeCarteScores.js
...
(async () => {
  ...
  if (!state) {
    console.log("Aucune partie La Juste Carte active pour le moment.");
    return;
  }

  const catalog = await loadCatalog();
  const entry = catalog.find((c) => c.cardKey === state.gameId);
  ...
  console.log(
    `Jeu La Juste Carte — Manche ${state.seasonManche}/${state.seasonMancheTotal} (${entry?.fr ?? state.gameId}) — Saison ${state.seasonId}\n`,
  );

  const touchedIds = new Set([...gameRanking.map((e) => e.discordId), ...inProgress.map((p) => p.discordId)]);
