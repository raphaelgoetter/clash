#!/usr/bin/env node
// postGobelet.js
// Poste manuellement (ou via cron) le jour du Jeu du Gobelet. Clôture
// d'abord le jour actif (s'il y en a un) : résout toutes les mains, attribue
// les points, puis publie le jour suivant (ou la révélation finale).
//
// Usage :
//   node scripts/postGobelet.js                — poste sur le salon de test
//   node scripts/postGobelet.js --public        — poste sur le salon public
//   node scripts/postGobelet.js --dry-run       — simulation, sans écrire ni poster
//   node scripts/postGobelet.js --public --dry-run
//   node scripts/postGobelet.js --no-ping       — poste sans pinger @MINI JEUX
//   node scripts/postGobelet.js --require-active — ne fait rien si aucune partie n'est déjà lancée (cron)
//   node scripts/postGobelet.js --force          — ignore le garde-fou anti-double-avancée

import dotenv from "dotenv";
dotenv.config({ path: "./.env" });

import { postGobelet } from "../api/discord/_handlers/gobelet.js";

const DRY_RUN = process.argv.includes("--dry-run");
const PUBLIC = process.argv.includes("--public");
// Réservé au cron quotidien (voir .github/workflows/gobelet.yml) : le Jour 1
// doit toujours être déclenché manuellement (workflow_dispatch), le cron ne
// fait qu'avancer une partie déjà en cours.
const REQUIRE_ACTIVE = process.argv.includes("--require-active");
// Contourne le garde-fou anti-double-avancée (voir backend/services/gobelet.js,
// isTooSoonSinceLastClosure) — protège uniquement le salon public contre un
// cron en retard après une relance manuelle. Sur le salon de test, aucun
// cron ne peut jamais entrer en conflit : le garde-fou n'y sert à rien
// d'autre qu'à ralentir l'itération manuelle, donc contourné automatiquement.
const FORCE = process.argv.includes("--force") || !PUBLIC;
// Jamais de ping sur le salon de test, même sans --no-ping explicite — seul
// le salon public ping réellement @MINI JEUX.
...
(async () => {
  try {
    const result = await postGobelet(channelId, {
      dryRun: DRY_RUN,
      noPing: NO_PING,
      isPublic: PUBLIC,
    ...
    if (result.wrongChannel) {
      console.error(
        `Une partie est déjà active sur un AUTRE salon (${result.activeChannelId}) — rien n'est posté ici pour éviter tout mélange. ` +
          `Si c'était une partie de test oubliée, lance "npm run gobelet:reset" puis relance sur le bon salon.`,
      );
      process.exit(1);
    }
