...
function buildReliabilityFields(score) {
  ...
  const lines = orderedBreakdown.map((item) => {
    ...
    return `${badge} ${item.label} (${item.score}/${item.max})`;
  });
    ...
    fields: [
      {
        ...
        value: [
          ...
          `⚔️ Counters directs (±12.5) : ${fmtLayer(detail.breakdown?.layer2)}% (${detail.reasons?.layer2 ?? "?"})`,
          `🏗️ Structure du deck (±7.5) : ${fmtLayer(detail.breakdown?.layer3)}% (${detail.reasons?.layer3 ?? "?"})`,
          `📊 Écart de niveau (±25) : ${fmtLayer(detail.breakdown?.layer4)}% (${detail.reasons?.layer4 ?? "?"})`,
        ].join("\n"),
        inline: false,
      },
      {
