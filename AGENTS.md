...
- `snapshot.js` écrit par défaut dans `/tmp/clash-snapshots/` mais persiste aussi dans `data/snapshots/` quand le dossier est accessible, afin de conserver les données entre les exécutions du cron.
  - `loadSnapshots()` lit `/tmp` en priorité et fusionne les données avec `data/snapshots/` si les deux sources existent.
  - La fusion se fait jour par jour via `mergeSnapshotsByDay()`, en préférant le snapshot de la journée avec le timestamp le plus récent valide.
...
- **Projection de Guerre (GDC)** : Calculée uniquement en `periodType: warDay`.
  ...
  - **Efficacité ($E$)** : `periodPoints` ÷ `decksToday` (jour courant) ; fallback historique `rivalAvgPtsPerDeckByTag` si `decksToday === 0`.
  - **Formule** : $Projection = max(Decks\_Aujourd\text{'}hui, T) \times E$ (équivalent à $clanScore + max(0, T - Decks\_Aujourd\text{'}hui) \times E$).
